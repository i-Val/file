...
class DonationsController extends Controller
{
	...
	public function paystack($id, $slug = null)
    {
	   ...
	   if( !isset( $deadline ) && $response->finalized == 1 ) {
		...
		if( str_slug( $response->title ) == '' ) {
			...
			$url_campaign = 'donate/'.$response->id.$slugUrl.'/paystack';
	...
	public function flutterwave($id, $slug = null)
    {
	   ...
	   if( !isset( $deadline ) && $response->finalized == 1 ) {
		...
		if( str_slug( $response->title ) == '' ) {
			...
			$url_campaign = 'donate/'.$response->id.$slugUrl.'/flutterwave';
		...
		return view('default.flutterwave')
				->with([
					'response' => $response,
					'pledge' => $pledge,
					'percentage' => $percentage,
